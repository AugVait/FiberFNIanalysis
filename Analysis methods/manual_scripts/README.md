...
- `manual_wavelength_cut_summary_plots.py`: plot sample-wise point grids for rise and decay fit times under `summary grids`, with editable manual-selection matrices, wavelength centers on the x-axis in nm, real fiber names in titles, and decay grids using a 1-3 ns color scale with values above 3 ns marked as outliers.
