...
CONFIG = METHODS_DIR / "configs" / "it_decay_fits_10ns.yaml"
...
def main() -> None:
    ...
    if TRACE_CONFIG_DIR is not None:
        args.extend(["--trace-config-dir", str(TRACE_CONFIG_DIR)])
    if OUT_SUBDIR is not None:
        args.extend(["--out-subdir", str(OUT_SUBDIR)])
    if TIME_WINDOW is not None:
        args.extend(["--time-window", str(TIME_WINDOW)])
    if REFRESH_CONFIGS:
