...
def plot_decay_fit(path: Path, time_ns: np.ndarray, counts: np.ndarray, row: pd.Series, title: str) -> None:
    ...
    ax.plot(x_dense, y_dense, color=COLORS["vermillion"], lw=1.45, label="decay fit")
    ax.text(
        0.98,
        0.95,
...
def plot_rise_fit(path: Path, time_ns: np.ndarray, counts: np.ndarray, row: pd.Series, title: str) -> None:
    ...
    ax.axvspan(fit_start, fit_end, color=COLORS["vermillion"], alpha=0.08, lw=0)
    ax.plot(x_dense, y_dense, color=COLORS["vermillion"], lw=1.45, label="rise fit")
    rise_time = float(row["fitted_rise_time_10_90_ns"])
    ax.text(
        0.98,
        0.95,
        f"10-90% = {rise_time:.3g} ns\nk = {sigmoid_k:.3g} ns\nR^2 = {float(row['r_squared']):.3f}",
        transform=ax.transAxes,
        va="top",
        ha="right",
        fontsize=7.5,
...
def forced_decay_fit_row(row: pd.Series, time_ns: np.ndarray, counts: np.ndarray) -> pd.Series:
    ...
    amplitude0 = max(float(y_fit[0] - baseline0), 1.0e-6)
    duration = max(float(x_fit[-1] - x_fit[0]), 0.03)
    tau0 = min(max(duration / 3.0, 0.03), 200.0)
    upper_baseline = max(float(np.nanmax(y_fit)), baseline0, 1.0e-9)
    ...
    try:
        popt, pcov = curve_fit(
            ...
            bounds=([0.0, 0.03, 0.0], [np.inf, 200.0, upper_baseline]),
        ...
        popt = np.array([amplitude0, tau0, baseline0], dtype=float)
...
def process_fit_family(
    *,
    ...
    plot_dir = family_dir / safe_scan_folder(entry["output_folder"]) / "fit_plots"
    for _, row in summary.iterrows():
        ...
        if fit_type == "rise":
            ...
            row = forced_decay_fit_row(row, time_ns, counts)
