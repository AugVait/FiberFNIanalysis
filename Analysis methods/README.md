...
To review all fit results by sample, run `manual_scripts/manual_wavelength_cut_summary_plots.py`. It writes point-grid summary plots under `../Analysis results/wavelength_cut_fit_results_2ns_rise_10ns_decay/summary grids`, with wavelength centers on the x-axis in nm and fiber positions as rows. Plot titles use the real fiber names from `configs/fiber_names.yaml`. Editable selection matrices are written under `../Analysis results/wavelength_cut_fit_results_2ns_rise_10ns_decay/manual selections`; positive cells are included in the plot and `0` or blank cells are excluded. The initial selection omits repeat-scan positions ending in `R`; replicate files at the same position and wavelength center are averaged into one point, and decay grids use a 1-3 ns color scale with values above 3 ns marked as outliers.
